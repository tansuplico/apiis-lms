// src/components/layout/facilitators/FacilitatorCourseLayout.tsx
...
export default function FacilitatorCourseLayout() {
  // ── Store
  ...
  useEffect(() => {
    if (!course || !course.canManage || expandedModule === null) return;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      if (
        ...
        setTimeout(() => {
          navigate(
            `/facilitator/course/${courseSlug}/module-${safeModuleNum}/${firstPartSlug}`,
          );
        }, 100);
      } else if (
  ...
  const saveWeights = async (weights: Record<number, string>) => {
    ...
    const equalShare =
      blankEntries.length > 0 && remaining > 0
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    >
                      {isEditMode && editingModuleId === mod.id ? (
                        ...
                        >
                          <span
                            className={`truncate max-w-36 block text-sm ${
                              isEditMode
                      ...
                      >
                        {isEditMode && editingPartId === part.id ? (
                          ...
                          <div className="flex-1 flex items-center gap-1.5 min-w-0">
                            <Link
                              to={`/facilitator/course/${courseSlug}/module-${mod.number}/${part.slug}`}
